...
## 📕 프로젝트 구조
![image](https://user-images.githubusercontent.com/64779472/146966352-ae09f2cf-61ec-4205-b9a4-f6d44e119f4d.png)
...
- redux: 리덕스 관련 폴더
- stories: 스토리북 관련 폴더
- styles: global style 관련 폴더
- utils: 유틸리티 관련 폴더
- assets: 정적 파일(이미지) 관련 폴더 <위 이미지에는 없음>
...
- Container - Presenter 디자인 패턴의 핵심은 데이터 처리와 데이터 출력을 분리하여 코드를 작성하는 것이다.
...
3. 선언적 확장(Declaration Merging), 자동 확장
```ts
  interface Person {
    name: string
    age: number
...
### 결론
- 프로젝트 내에서 union이나 tuple 등이 필요한 경우를 제외하면 웬만하면 Interface를 사용한다.
...
- saga난 thunk로 못하는 다양한 작업들을 처리할 수 있다.
...
## ✨ 7. immer(도입 고민중)
- immer란? react에서 불변성을 유지하는 코드를 작성하기 쉽게 해주는 라이브러리
...
### react의 기본 속성
- react는 기본적으로 부모 컴포넌트가 리렌더링을 하면 자식 컴포넌트도 리렌더링된다. 즉 얕은 비교를 통해 새로운 값인지 아닌지 판단 후에 새로운 값인 경우 리렌더링을 한다.
...
```
- 즉 위와 같은 이유로 우리가 state를 바꾸고 돔을 다시 만들려면, 새로운 객체 or 배열을 만들어 새로운 참조값을 만들고, react에게 이 값은 이전과 다른 참조값임을 알려줘야 한다.
